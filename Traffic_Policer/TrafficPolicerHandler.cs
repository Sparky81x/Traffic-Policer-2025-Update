...
[assembly: Rage.Attributes.Plugin("Traffic Policer", Description = "INSTALL IN PLUGINS/LSPDFR. Enhances traffic policing in LSPDFR", Author = "Albo1125 Updated by Pherem")]
namespace Traffic_Policer
{
    /// <summary>
    /// Entry point for the Traffic Policer plugin.
    /// </summary>
    public class EntryPoint
    {
        /// <summary>
        /// Main method executed when the plugin is loaded.
        /// </summary>
    ...
    {
        // Stores references to ambient event GameFibers so they can be aborted cleanly
        public static List<GameFiber> AmbientEventGameFibersToAbort = new List<GameFiber>();

        // Peds flagged for DUI charges
        ...
        // Centralized RNG for event decisions
        private static readonly Random eventRnd = new Random();


        private static void eventCreator()
        {
            // Ensure list is initialized
            if (driversConsidered == null)
            ...
            GameFiber.StartNew(() =>
            {
                try
                {
                    while (true)
                    {
                            ...
                            NextEventStopwatch.ElapsedMilliseconds >= nextEventTimer)
                        {
                            ...
                            {
                                if (!car.Exists() || Ambientevents.BrokenDownVehicle.BrokenDownVehicleRunning || NextEventStopwatch.ElapsedMilliseconds < nextEventTimer)
                                    break;
                                ...
                                if (car.HasDriver && car.Driver.Exists() && !car.IsPoliceVehicle && !car.HasSiren)
                                {
                                    ...
                                    // Filter out invalid or already-used drivers
                                    if (driver == null || !driver.Exists() || driver == Game.LocalPlayer.Character || driversConsidered.Contains(driver))
                                        continue;

                                    // Attempt to trigger ambient events
                                    ...
                                    // Track driver so we don't reassign
                ...
                {
                    Game.LogTrivial($"[Traffic Policer] ERROR in eventCreator fiber: {ex.Message}\n{ex.StackTrace}");
                }
            });
        }
