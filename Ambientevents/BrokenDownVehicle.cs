...
namespace Traffic_Policer.Ambientevents
{
    ...
    "FELON", "FELON2", "ZENTORNO", "WARRENER", "RAPIDGT", "INTRUDER", "FELTZER2", "FQ2", "RANCHERXL", "REBEL",
    "SCHWARZER", "COQUETTE", "CARBONIZZARE", "EMPEROR", "SULTAN", "EXEMPLAR", "MASSACRO", "DOMINATOR", "ASTEROPE",
    "PRAIRIE", "NINEF", "WASHINGTON", "CHINO", "CASCO", "INFERNUS", "ZTYPE", "DILETTANTE", "VIRGO", "F620", "PRIMO",
    "SENTINEL", "WINDSOR", "GAUNTLET", "BUFFALO"
        };
        ...
        public BrokenDownVehicle(bool createBlip, bool displayMessage)
        {
            ...
            {
                float distance = Vector3.Distance(tuple.Item1, Game.LocalPlayer.Character.Position);
                if (distance < 300f && distance > 140f)
                {
                    float travelDist = NativeFunction.Natives.CALCULATE_TRAVEL_DISTANCE_BETWEEN_POINTS<float>(
                        tuple.Item1.X, tuple.Item1.Y, tuple.Item1.Z,
                        Game.LocalPlayer.Character.Position.X,
                        Game.LocalPlayer.Character.Position.Y,
                        Game.LocalPlayer.Character.Position.Z);
            ...
            if (ValidTrafficStopSpawnPointsWithHeadings.Count > 0)
            {
                ...
                // Pick a random model
                string model = vehiclesToSelectFrom[TrafficPolicerHandler.rnd.Next(vehiclesToSelectFrom.Length)];

                // Create vehicle and driver
                car = new Vehicle(model, SpawnPoint, SpawnHeading);
                car.MakePersistent();
                ...
                if (createBlip)
                {
                    ...
                    driverBlip.Color = System.Drawing.Color.Blue;
