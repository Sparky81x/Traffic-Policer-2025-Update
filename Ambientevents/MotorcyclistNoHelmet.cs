    ...
    internal class MotorcyclistNoHelmet : AmbientEvent
    {
        ...
        public MotorcyclistNoHelmet(bool createBlip, bool displayMessage) : base(displayMessage, "Creating motorcyclist without a helmet event.")
        {
            try
            {
                ...
                string model = bikesToSelectFrom[TrafficPolicerHandler.rnd.Next(bikesToSelectFrom.Length)];
        ...
        protected override void MainLogic()
        {
            AmbientEventMainFiber = GameFiber.StartNew(delegate
            {
                try
                {
                    ...
                    {
                        API.LSPDFRPlusFunctions.AddQuestionToTrafficStop(driver, "Where was your helmet?", new List<string>
                        {
                            "Sorry, officer. I forgot",
                            "It's a bit uncomfortable to wear, officer.",
                            "What the hell do you care?",
                            "I was enjoying the lovely weather, officer."
                        });
                    }
                ...
                {
                    Game.LogTrivial(e.ToString()); // Log the exception to use the variable 'e'
                    eventRunning = false;
