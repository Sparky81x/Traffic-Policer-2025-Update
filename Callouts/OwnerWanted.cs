...
namespace Traffic_Policer.Callouts
{
    ...
    internal class OwnerWanted : Callout
    {
        ...
        public override bool OnCalloutAccepted()
        {
            ...
            // Correct license plate format
            PlateFormat format = LicensePlateGenerator.GetRandomPlateFormat();
            string plate = LicensePlateGenerator.GenerateLicensePlate(format);
        ...
        public override void Process()
        {
            ...
            if (!displayedModelName)
            {
                if (Vector3.Distance(Game.LocalPlayer.Character.Position, car.Position) < 35f)
                {
                    ...
                    string article = vowels.Contains<string>(modelName[0].ToString()) ? "an" : "a";
        ...
        public override void End()
        {
            ...
            {
                Game.LogTrivial($"Exception caught: {e.Message}");
            ...
            GameFiber.StartNew(delegate
            {
                try {
                    ...
                    while (TrafficPolicerHandler.isOwnerWantedCalloutRunning)
                    {
                        ...
                        {
                            Game.LogTrivial(e.ToString()); // Log the exception to avoid the unused variable warning
                            continue; // Retain the existing behavior
                        }
        ...
        private void callForReinforcements(Ped caller)
        {
            ...
            Vector3 spawnPoint = World.GetNextPositionOnStreet(caller.Position.Around2D(50f));
            ...
            PlateFormat format = LicensePlateGenerator.GetRandomPlateFormat();
            string plate = LicensePlateGenerator.GenerateLicensePlate(format);
            reinforcementCar.LicensePlate = plate;
            ...
            reinforcementCarBlip = reinforcementCar.AttachBlip();
            reinforcementCarBlip.Color = System.Drawing.Color.Red;
            ...
            GameFiber.StartNew(delegate
            {
                try
                {
                    ...
                    while (TrafficPolicerHandler.isOwnerWantedCalloutRunning)
                    {
                        ...
                        if (Vector3.Distance(reinforcementCar.Position, callerPosition) < 15f ||
                            Vector3.Distance(reinforcementCar.Position, Game.LocalPlayer.Character.Position) < 15f ||
                            !reinforcementCar.Exists())
                            break;
                        ...
                        if (drivingCount == 300)
                        {
                            ...
                            while ((Vector3.Distance(newspawnPoint, Game.LocalPlayer.Character.Position) < 20f) ||
                                   (Math.Abs(newspawnPoint.Z - Game.LocalPlayer.Character.Position.Z) > 4f))
                            {
                    ...
                    while (TrafficPolicerHandler.isOwnerWantedCalloutRunning)
                    {
                        if (reinforcementCarDriver.Exists())
                        {
                            ...
                            else if (!Functions.IsPedGettingArrested(reinforcementCarDriver) &&
                                     !Rage.Native.NativeFunction.Natives.IS_PED_IN_COMBAT<bool>(reinforcementCarDriver))
                            {
                                reinforcementCarDriver.Tasks.ClearImmediately();
                                Rage.Native.NativeFunction.Natives.TaskCombatPed(reinforcementCarDriver, Game.LocalPlayer.Character, 0, 16);
                            }
                        ...
                        if (passenger1.Exists())
                        {
                            if (passenger1.IsDead || Functions.IsPedArrested(passenger1))
                                passenger1Dead = true;
                            else if (!Functions.IsPedGettingArrested(passenger1) &&
                                     !Rage.Native.NativeFunction.Natives.IS_PED_IN_COMBAT<bool>(passenger1))
                            {
                                passenger1.Tasks.ClearImmediately();
                                Rage.Native.NativeFunction.Natives.TaskCombatPed(passenger1, Game.LocalPlayer.Character, 0, 16);
                            }
                        ...
                        if (passenger2.Exists())
                        {
                            if (passenger2.IsDead || Functions.IsPedArrested(passenger2))
                                passenger2Dead = true;
                            else if (!Functions.IsPedGettingArrested(passenger2) &&
                                     !Rage.Native.NativeFunction.Natives.IS_PED_IN_COMBAT<bool>(passenger2))
                            {
                                passenger2.Tasks.ClearImmediately();
                                Rage.Native.NativeFunction.Natives.TaskCombatPed(passenger2, Game.LocalPlayer.Character, 0, 16);
                            }
                        ...
                        if (Game.LocalPlayer.Character.IsDead ||
                            (reinforcementDriverDead && passenger1Dead && passenger2Dead) ||
                            (!reinforcementCarDriver.Exists() && !passenger1.Exists() && !passenger2.Exists()))
                            break;
                ...
                catch (Exception e)
                {
                    Game.LogTrivial(e.ToString());
                    Game.LogTrivial("Traffic Policer handled the exception successfully.");
                    if (reinforcementCarBlip != null && reinforcementCarBlip.Exists())
                        reinforcementCarBlip.Delete();
